...
TREND ANALYSIS
7️⃣ Total Sales by Year, Month and Day
...
8️⃣ Sum of Return ID by Year, Quarter, Month and Day
SELECT 
    return_date,
...
🔟 Sum of Sales by Age Group (Pie Chart)
SELECT 
    CASE 
        WHEN age < 25 THEN '<25'
...
1️⃣1️⃣ Total Sales by Age Group (Bar Chart)
SELECT 
    CASE 
        WHEN age < 25 THEN '<25'
        WHEN age BETWEEN 25 AND 34 THEN '25-34'
...
1️⃣2️⃣ Sum of Sales by Payment Method
SELECT 
    payment_method,
    SUM(sales_amount) AS total_sales
FROM sales
...
1️⃣3️⃣ Top 8 Product Categories by Sales
SELECT 
    p.category_name,
    SUM(s.sales_amount) AS total_sales
FROM sales s
...
1️⃣4️⃣ Top 5 Products by Sales
SELECT 
    p.product_name,
    SUM(s.sales_amount) AS total_sales
FROM sales s
...
1️⃣5️⃣ Sum of Returned Orders & Total Orders by Category
SELECT 
    p.category_name,
